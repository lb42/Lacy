...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="corpus">/TEI</xsl:param>
    <!-- the language code -->

    <xsl:param name="webRoot">https://distantreading.github.io/Lacy/</xsl:param>
    <!-- target website -->

    <xsl:param name="repoRoot">/home/lou/Public/Lacy/TEI</xsl:param>
    <!-- source files -->
    <xsl:param name="gitRoot">https://raw.githubusercontent.com/lb42/Lacy</xsl:param>
    <!-- git home of the source files -->

    <xsl:variable name="gitRootRepo">
        <xsl:value-of select="concat($gitRoot, $corpus, '/master')"/>
    </xsl:variable>

    <!-- these two files are generated by refresh.py -->

    <xsl:variable name="driverFile">
        <xsl:value-of select="concat($repoRoot, $corpus, '/driver.tei')"/>
    </xsl:variable>
    <xsl:variable name="fileNameFile">
        <xsl:value-of select="concat($repoRoot, $corpus, '/fileNames.xml')"/>
    </xsl:variable>
    ...
    <xsl:template match="/">
        ...
        <html>
            <head>
                ...
                <title>
                    <xsl:value-of select="concat('Contents of Lacy', $corpus)"/>
                </title>
            </head>
            <body>
                ...
                <xsl:variable name="status">
                    ...
                    <xsl:value-of select="concat('Lacy/',$corpus)"/>
    ...
    <!--<xsl:message><xsl:value-of select="count(t:teiCorpus/t:TEI/t:teiHeader)"/> headers</xsl:message>-->
     <xsl:for-each select="t:teiCorpus/t:TEI/t:teiHeader">
                        ...
                        <xsl:variable name="wc">
                            <xsl:choose>
                                <xsl:when test="t:fileDesc/t:extent/t:measure[@type = 'words']">
                                    <xsl:value-of   select="normalize-space(t:fileDesc/t:extent/t:measure[@type = 'words']/@quantity)"/>
                                </xsl:when>
...
<xsl:value-of select="t:fileDesc/t:sourceDesc/t:bibl[@type = 'source']/t:eventName[@type='firstPerf']/@notAfter"/>                                
                        </xsl:variable>
                        ...
                        <xsl:text>,</xsl:text>
                        <xsl:if test="t:profileDesc/t:textDesc/e:reprintCount">
                            <xsl:value-of select="t:profileDesc/t:textDesc/e:reprintCount/@key"/>
                        </xsl:if>
                        <xsl:if test="t:profileDesc/t:textDesc/e:canonicity">
                ...
                <!-- now display mosaic graphic (which will be created after this script runs) and text details -->
                ...
                <table class="catalogue" id="theTable">
                    ...
                    <xsl:for-each select="t:teiCorpus/t:TEI/t:teiHeader">
                        ...
                        <xsl:variable name="targetFileName">
                            <xsl:value-of select="concat($webRoot, $corpus, '/', $textID, '.html')"
                            />
                        </xsl:variable>
                        <!-- this is a link to the xml source in the git repo: get its name from the filenames file -->
                        <xsl:variable name="sourceFile">
                            <xsl:value-of
                                select="document($fileNameFile)//file[matches(., $textID)]"/>
                        </xsl:variable>
                        <!-- and off we go -->
                        <tr>
                            <xsl:variable name="wc">
                            ...
                            <td>
                                ...
                                <a>
                                    <xsl:attribute name="href">
                                        <xsl:value-of
                                            select="concat($gitRoot, $corpus, '/master/', $sourceFile)"
                                        />
                                    </xsl:attribute>
                            ...
                            <td>
                                <xsl:choose>
                                    <xsl:when
                                        test="contains(t:fileDesc/t:titleStmt/t:title[1], ':')">
                                        <xsl:value-of
                                            select="substring-before(t:fileDesc/t:titleStmt/t:title[1], ':')"
                                        />
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:value-of
                                            select="substring-before(t:fileDesc/t:titleStmt/t:title[1], 'ELT')"
                                        />
                                    </xsl:otherwise>
                                </xsl:choose>
