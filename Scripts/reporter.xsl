...
<!-- produce nice HTML table showing select metadata from the catalogue file -->
...
<xsl:param name="gitRoot"
>https://raw.githubusercontent.com/lb42/Lacy/main/TEI/</xsl:param>
...
<xsl:message>Processing driver file <xsl:value-of select="$driverFile"
/></xsl:message>
...
<title>
<xsl:text>Lacy's Acting Edition of Plays</xsl:text>
</title>
</head>
<body>
...
<xsl:value-of
select="format-number(sum(//t:measure[@type = 'txWords']/@quantity), '#')"/>
</xsl:variable>
...
<p class="explain">Click on a column heading to sort. Click on a text identifier
to read the text (may not work in older browsers).</p>
...
<xsl:value-of select="concat($gitRoot, '/', $textID, '.xml')"/>
</xsl:variable>
<!-- and off we go -->
<tr>
...
<xsl:value-of select="$thisOne//t:extent/t:measure[@type = 'pp']/@quantity"/>
</xsl:variable>
<xsl:variable name="date"
select="$thisOne/t:sourceDesc/t:listEvent/t:event[@type = 'firstPerf']/@when"/>
<xsl:variable name="venu"
...
<td>
<xsl:value-of select="$thisOne/t:titleStmt/t:title[1]"/>
</td>
<td>
...
<td>
<xsl:value-of
select="following-sibling::t:profileDesc/t:textClass/t:catRef/@target"/>
</td>
<td>
<xsl:value-of
select="$thisOne/following-sibling::t:profileDesc/t:textClass/t:keywords/t:term"/>
</td>
</tr>
</xsl:for-each>
</table>
