#!/usr/bin/env python3
"""
tei_tagger_stdlib.py
====================
Same as tei_tagger.py but uses only Python standard library modules
(urllib, json, etc.) — no pip install required.

Usage
-----
    python tei_tagger_stdlib.py [options]
...
# ---------------------------------------------------------------------------
# Entry detection  (identical to tei_tagger.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Prompt  (identical to tei_tagger.py)
# ---------------------------------------------------------------------------
...
4. Also add  TEI elements as follows: each **entry** should be wrapped in a <div type='entry'>; each distinct **heading** should be wrapped in a <head>; each distinct **paragraph** should be wrapped in a <p>; insert an empty <pb/> element at the start of each page. . No other TEI tags should be added.   Return the text with ONLY the tag types above inserted.

5. Preserve the original spelling, punctuation, line-breaks, and wording
   exactly, but correct obvious OCR errors. Do  not paraphrase.
...
# ---------------------------------------------------------------------------
# TEI wrapper  (identical to tei_tagger.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# API call using urllib (no third-party packages)
# ---------------------------------------------------------------------------
...
def call_api(api_key: str, model: str, system: str, user_text: str,
             max_tokens: int = 8192) -> str:
    """POST to the Anthropic Messages API and return the assistant's text."""
    payload = json.dumps({
        "model": model,
...
def tag_chunk(
    ...
    for attempt in range(1, retries + 1):
        try:
            return call_api(api_key, model, SYSTEM_PROMPT, user_msg)
        except urllib.error.HTTPError as exc:
            status = exc.code
            body = exc.read().decode("utf-8", errors="replace")
            print(f"  HTTP {status} (attempt {attempt}/{retries}): {body[:200]}")
            if status == 529 or status == 429:          # overloaded / rate-limited
                if attempt < retries:
...
# ---------------------------------------------------------------------------
# Progress tracking  (identical to tei_tagger.py)
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    for i, chunk_entries in enumerate(chunks):
        ...
        try:
            tagged = tag_chunk(api_key, args.model, chunk_entries, i, total_chunks)
        except Exception as exc:
