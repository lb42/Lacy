...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:template match="/">
 <!-- <xsl:text>id,title,pp,txWords,spCount,roles,rolesM,rolesF,genre,class
</xsl:text>-->
  <xsl:for-each select="TEI">
   <xsl:variable name="qq">"</xsl:variable>
   <xsl:value-of select="@xml:id"/><xsl:text>,"</xsl:text>
   <xsl:value-of select="normalize-space(replace(teiHeader/fileDesc/titleStmt/title[1], $qq,''))"/><xsl:text>",</xsl:text>
   ...
   <xsl:value-of select="count(text/body//sp)"/><xsl:text>,</xsl:text>
   ...
   <xsl:value-of select="teiHeader/profileDesc/textClass/keywords"/><xsl:text>
   </xsl:text>
</xsl:for-each>
